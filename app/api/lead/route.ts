...
function buildTelegramMessage(data: ReturnType<typeof validateLeadForm> & { valid: true }): string {
  const d = data.data
  return [
    `🏠 <b>Nowy lead — Kredyt dla Ukraińców</b>`,
    ``,
    `👤 <b>Imię:</b> ${field(d.name)}`,
    `📞 <b>Telefon:</b> ${field(d.phone)}`,
    `📧 <b>E-mail:</b> ${field(d.email)}`,
    ``,
    `🇺🇦 <b>Status pobytu:</b> ${field(d.situation)}`,
    `💼 <b>Źródło dochodu:</b> ${field(d.income)}`,
    `🏦 <b>Rodzaj kredytu:</b> ${field(d.creditType)}`,
    ``,
    `📝 <b>Opis sytuacji:</b>`,
    field(d.description),
    ``,
